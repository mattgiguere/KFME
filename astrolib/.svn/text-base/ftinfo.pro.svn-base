...
;
 ...
 ttype = sxpar(h,'ttype'+apos,Count=N_ttype)                    ;field name
 if N_ttype EQ 0 then begin
        apos = strmid(string(fpos+1000,format = '(I4)'),1,3)
        ttype = sxpar(h,'ttype'+apos, Count = N_ttype)
 end
 if N_ttype EQ 0 then $
        message,'Invalid FITS table header -- keyword TTYPE'+apos+' not present'
;
 tbcol = sxpar(h,'tbcol'+apos, Count = N_tbcol)       ;starting column position
 if N_tbcol EQ 0 then $
        message,'Invalid FITS table header -- keyword TBCOL' + $
                          apos+' not present'
;
 tform = sxpar(h,'tform'+apos, Count = N_tform)                 ; column format
 if N_tform EQ 0 then $
        message,'Invalid FITS table header -- keyword TFORM'+apos+' not present'
;                                               ; physical units
 tunit = sxpar(h, 'TUNIT' + apos, Count = N_tunit)
 if N_tunit EQ 0 then tunit = ''
;
 tscal = sxpar(h, 'TSCAL' + apos, Count = N_tscal)      ; data scale factor
 if N_tscal EQ 0 then tscal = 1
;
 tzero = sxpar(h,'TZERO' + apos, Count = N_tzero)       ; zero point for field
 if N_tzero EQ 0 then tzero = 0
;
 tnull = sxpar(h,'TNULL' + apos, Count = N_tnull)       ;null data value
 if N_tnull EQ 0 then tnull = ''
;
