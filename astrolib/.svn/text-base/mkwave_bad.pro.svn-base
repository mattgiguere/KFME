...
;Extract info from wvc information block.
  ...
  ncross = wvc(7)				;number of cross-terms
  coldeg = wvc(8)				;degree of column fit poly
  orddeg = wvc(9)				;degree of order fit poly
...
;Loop thru orders, building wavelength array.
  ...
  for i=0,no-1 do begin				;loop thru build orders
    ...
    if vers ge 2.2 then begin			;cross-terms added
      inx = coldeg + orddeg +1			;start of cross-terms
      nlx = order*ic *( coeff(inx) + ic*coeff(inx+1) $
	+ order*coeff(inx+2) + ic*order*coeff(inx+3) )
    endif else nlx = 0.0			;else: no cross terms
    if vers ge 2.5 then begin			;cross-terms added
      nlx = nlx + order*ic *( order*order*coeff(inx+4) $
                            + ic*ic*coeff(inx+5))
    endif
