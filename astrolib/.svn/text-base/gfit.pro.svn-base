;This is gaussfit.pro from the standard IDL library, modified so that the
;  baseline under the gaussian is only allowed to be linear. [JAV 1-Jun-92]

PRO	G_FUNCT,X,A,F,PDER
; NAME:
;	G_FUNCT
;
; PURPOSE:
...
;	A3 = constant term, A4 = linear term
...
a = [yd(i0), x(i0), abs(x(i0)-x(i0+i)), c(0), c(1)] ;estimates
!c=0				;reset cursor for plotting
return,curvefit(x,y,replicate(1.,n),a,sigmaa, $
		function_name = "G_FUNCT") ;call curvefit
end

