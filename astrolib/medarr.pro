PRO medarr, inarr, outarr, mask, output_mask, progress=progress
;+
; NAME:
;       MEDARR
; PURPOSE:
...
;       Changed conditional at line 94         JohnJohn January 2003
;-
 On_error,2
;                       Check parameters.
...
;                       Combine the input arrays into the output array.
 ...
 if cw gt 0 and n_elements(sm) gt 3 then begin
    w = where(sm[0:3] eq s[0:3], cw)
    if cw eq 4 then begin
       mask_given = 1b 
    endif else begin
       print,'MEDARR:  Mask not valid... must be same shape as input cube.'
       return
    endelse
 endif
 ...
 if not mask_given then begin
     for j = 0l, nrow-1 do begin
         ...
         if keyword_set(progress) then counter, j,nrow,/per
     endfor
 endif else begin
