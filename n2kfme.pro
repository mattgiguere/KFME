...
pro mpf_widget_dofit, pstate
  ...
  if (*pstate).psplot then begin
	 ...
	 filen=nextnameeps('~/kfme_output/plots/rvwidgetplot', '')
...
;  stop
  ...
  for j=0,ntrial-1 do begin
	  ...
	  dum = where(chiarr lt chi_init, fas)
...
;endif
 ...
 tlb = widget_base(title = 'Interactive N2KFME v. 2011/09/01 ', $
...
n2krow = widget_base(controlbase, /row)
restore, '/exolib/KFME/n2k_aug2011.dat'
;Make a droplist to hold line style choices:
; symbols = ['SYMBOL', 'Plus', 'Asterisk', 'Period', 'Diamond', $
; 'Triangle', 'Square', 'X', 'Solid Dot']
n2ktargs = ['N2K TARGS',strt(n2k.name)]
 
 n2kdrop = widget_droplist(n2krow, value=n2ktargs, $
 title = '', event_pro = 'kfme_n2ktargs', xsize=halfcol)
