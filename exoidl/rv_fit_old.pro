...
;INPUT
;      cf           Optional input cf array.  This overrides keyword "star"
...
;     frz_ecc       value to which the eccentricity will be FROZEN:  e = frzecc
;     errcut       thresh = errcut * median(errvel) --- for removal of poor points.
;     slide        invoke for blue background (for slides)
;     points       display points only, not fit (for overlay)
;     fit          display fit only, not points (works for unphased data only)
...
;            simply start the program below at "TIMES", with t, obvel, sig.
...
if keyword_set(web) and keyword_set(postscript) then begin
 print,'Both keywords, web and postscript, were set: Choose one.'
 stop
end

if keyword_set(web) then phase = 1  ;force phase for online plots.
...
if cf_arg_input eq 1 and n_elements(cf) eq 0 then begin
  ...
  print,'Please check that cf3 or cf1.  Perhaps your VST does not have it.'
  print,'Returning...'
  stop
end
...
velplot,inpcf,title,tbin,time,vel,errv,dum,nav,bincf,/noplot,errcut=errcut,nozero=nozero
;stop
...
dvel = 0.
if not keyword_set(nozero) then begin  ;don't muck with vel zero-poitn
...
if ~finite(p) then stop
print, 'p before is: ', p, 'line 282 in rv_fit.'
;GUESS PARAMETERS
...
if keyword_set(trend) then begin
  cof = poly_fit(t,obvel,1)
  dvdt = cof(1)
  if abs(dvdt) gt 1. then dvdt = 1.d0*dvdt/abs(dvdt)  ;set to + or - 1 m/s/day
...
if strtrim(string(minchi),2) eq 'NaN' then begin
    ...
    if imin eq 0 then pararr = rm_elements(pararr[*, 1:3])
    if imin eq 3 then pararr = rm_elements(pararr[*, 0:2])
    if ( (imin gt 0) AND (imin lt 3) ) then $
	  	pararr = [[rm_elements(pararr[*, 0:(imin-1)])], $
	  		[rm_elements(pararr[*, (imin+1):*])]] ;added 13Dec2008 ~MJG
...
;added 13Dec2008 ~MJG:
;Determine if any of the parameters are NaN and if so, remove them: 
while ~finite(pararr[0, imin]) do begin
    if imin eq 0 then pararr = rm_elements(pararr[*, 1:n_elements(chiarr)-1])
    if imin eq n_elements(chiarr)-1 then pararr = rm_elements(pararr[*, 0:n_elements(chiarr)-2])
    if ( (imin gt 0) AND (imin lt n_elements(chiarr)-1) ) then $
	  	pararr = [[rm_elements(pararr[*, 0:(imin-1)])], $
	  		[rm_elements(pararr[*, (imin+1):*])]] 
    remove,imin,chiarr
    minchi = min(chiarr,imin)     ;Use minimum Chi from 4 initial omega's
endwhile
...
if ~finite(par[0]) then stop
print, 'the period after is: ', par[0], ' line 482 in RV_FIT'
...
obnm1=bincf.obnm
  if n1 gt 0 and n2 gt 0 then begin
	 ...
	 if ((n1 le 0) and (n2 gt 0)) then begin
	 ...
	 if ((n2 le 0) and (n1 gt 0)) then begin
		  t = [tph(j1)-1., tph] 
...
endelse
 ...
 FOR j=0,numpts-1 do begin
     ...
     oplot,[t(j),t(j)],[obvel(j)+midwy-sig(j),obvel(j)+midwy+sig(j)],thick=6,col=200

     !p.color=0
     if t(j) gt 0.0 and t(j) le 1.0 then $
     oplot,[t(j),t(j)],[obvel(j)+midwy-sig(j),obvel(j)+midwy+sig(j)],thick=6,col=0
 END
...
;  if keyword_set(keck) then begin
;     xyouts,xr(1)-0.45,yr(0)+.05*leny,'!6 Keck',size=sz*0.7
;     xyouts,xr(1)-0.45,yr(0)+.05*leny,'!6 ',size=sz*0.7
;  end else begin
;     xyouts,xr(1)-0.45,yr(0)+.05*leny,'!6 Lick Obs.',size=sz*0.7
;  endelse


if keyword_set(web) then !p.charthick = 2
; if keyword_set(keck) then begin
;     xyouts,xr(1)-0.3,yr(0)+.05*leny,'!6 Keck',size=sz*0.9
; end
 if keyword_set(aat) then begin
     xyouts,xr(1)-0.4,yr(0)+.05*leny,'!6 AAT',size=sz*0.9
 end

; if (not keyword_set(aat)) and (not keyword_set(keck)) then begin
;     xyouts,xr(1)-0.4,yr(0)+.05*leny,'!6 Lick Obs.',size=sz*0.9
; end
