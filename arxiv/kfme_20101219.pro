...
;***********************************************************************
function mpf_widget_create_par, pstate
...
; n_parameters = 7.*n_planets + 9 + 3
 ...
 return, pararr
end;mpf_widget_create_par.pro

pro mpf_widget_retrieve_par, pstate, pararr, perror
...
end;mpf_widget_retrieve_par.pro

function mpf_rvdatelabel, axis, index, value
	;; Calls the global variables in the 'SharedVarRV' block
	;common SharedVarRV
	common SharedVarRV, toff2
	
	MonthArr = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']
	
	d0 = 2.44d6
	;print, 'toff2 is: ', toff2
	;print, 'value is: ', value
	caldat, long(value+d0+toff2), month, day, year
	year = strtrim(year,2)	
	lab = monthArr(month-1)+' '+year
	;print, 'lab is: ', lab
	return, lab
end

pro mpf_widget_dofit, pstate
  ...
  !p.thick = 2.
   ...
   pararr = mpf_widget_create_par(pstate)
  ...
  plot, fitobs, fitdat, $
	 ...
	 XTickFormat='mpf_rvdatelabel', xminor=4, XTICKINTERVAL = rvran/5, $
	 xtitle = 'Time of Observation', $
	 ytitl='Radial Velocity [m/s]', $
	 title=(*(*pstate).pfunctargs).extitle
...
if ~(*pstate).psplot then begin
  ...
  xyouts, .1, .02, 'real '+Greek('chi')+'!d'+Greek('nu')+$
  '!u2!n'+': '+strt(chisq), /norm
  ;rmsresid = stddev(fitdat-fity)
  xyouts, .3, .02, 'RMS: '+strt(rmsresid), /norm
  xyouts, .7, .02, 'STDDEV RV: '+$
	 strt(stddev((*(*pstate).pcf).cf_rv.mnvel)), /norm
endif
...
if (*pstate).tfine then begin
  ...
  oplot, tfine, tfitrv, color = 120, linestyle = 0, $
  		thick=4.
 ...
 if (*pstate).multith then mpf_widget_multiline, pstate
...
end;mpf_widget_dofit.pro

pro mpf_widget_multiline, pstate
...
end ;mpf_widget_multiline.pro

pro mpf_widget_update_fields, pstate
...
;Replot the data in the draw widget with the "do" routine:
 mpf_widget_dofit, pstate

end;mpf_widget_update_fields.pro

pro mpf_widget_resize, event
...
end;mpf_widget_resize.pro

pro mpf_widget_par, event
print, 'event.value is: ', event.value

end ;mpf_widget_par.pro

pro mpf_widget_cleanup, tlb
 widget_control, tlb, get_uvalue=pstate
 ptr_free, (*pstate).pcf
 ptr_free, pstate
end ;mpf_widget_cleanup.pro
 ...
 ;**************************************************************
pro mpf_widget_openlickvst, event
  ...
  mpf_widget_update_fields, pstate
...
end ;mpf_widget_openlickvst.pro

pro mpf_widget_openkeckvst, event
  ; Get the pointer to the state structure from the user value
  ; of the top-level base.
  widget_control, event.top, get_uvalue=pstate
  ...
  ;restore, '/exolib/data/planeten.dat'
...
end ;mpf_widget_openkeckvst.pro


pro mpf_widget_saveall, event
  ; Get the pointer to the state structure from the user value
  ; of the top-level base.
  widget_control, event.top, get_uvalue=pstate
...
end ;mpf_widget_saveall.pro

pro mpf_widget_restoreall, event
  ; Get the pointer to the state structure from the user value
  ; of the top-level base.
  widget_control, event.top, get_uvalue=pstate
...
end ;mpf_widget_restoreall.pro

pro mpf_widget_list, event
 ...
 mpf_widget_xminxmax, pstate
end ;mpf_widget_list.pro

pro mpf_widget_data, event
...
end ;mpf_widget_data.pro
  
pro mpf_widget_plotdata, event
...
end ;mpf_widget_plotdata.pro

pro mpf_widget_planetalone, event
...
end;mpf_widget_planetalone.pro

pro mpf_widget_aloneplot, event
  ...
  if (*pstate).psplot then begin
     !p.font=1
	 filen=nextnameeps('aloneplot', '', /nosuf)
...
if ~(*pstate).controlbar.phasebool then begin
  plot,tfine, tfitloner, $
	linestyle=(*pstate).linestyle, $
	color = 0., psym=(*pstate).psym*(*pstate).connect, $
	XTickFormat='mpf_rvdatelabel', xminor=4, XTICKINTERVAL = rvran/4, $
	xtitle = 'Time of Observation', $
	ytitl='Radial Velocity [m/s]', $
...
endelse
  ...
  xyouts, .1, .02, Greek('chi')+'!d'+Greek('nu')+$
  '!u2!n'+': '+strt(chisq), /norm
  xyouts, .3, .02, 'RMS: '+strt(rmsresid), /norm
  xyouts, .7, .02, 'STDDEV RV: '+$
	 strt(stddev((*(*pstate).pcf).cf_rv.mnvel)), /norm
  ...
  endelse
	...
	loadct, 39, /silent
end;mpf_widget_aloneplot.pro

pro mpf_widget_orbitphase, event
...
end;mpf_widget_orbitphase.pro

pro mpf_widget_sliderphase, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end ;mpf_widget_sliderphase.pro

pro mpf_widget_pergram, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
  ...
  mpf_widget_pergram_min, (*(*pstate).pcf).cf_rv, $
  /verbose, lowper=(*pstate).perglow, pmax=(*pstate).perghi, $
  numper=(*pstate).pergres, title = (*(*pstate).pcfname)+' RV '
...
end;mpf_widget_pergram.pro


pro mpf_widget_residpergram, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
  ...
  ;!p.charsize=3.
  print, 'Radial Velocity'
  mpf_widget_pergram_min, (*(*pstate).pcf_resid).cf_rv, $
  /verbose, lowper=(*pstate).perglow, pmax=(*pstate).perghi, $
  numper=(*pstate).pergres, title = (*(*pstate).pcfname)+' RV '
...
end;mpf_widget_residpergram.pro

pro mpf_widget_lowperg, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_lowperg.pro

pro mpf_widget_hiperg, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_hiperg.pro

pro mpf_widget_pergres, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_pergres.pro

pro mpf_widget_fap, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
  ...
  pergram,cf3,nu_out,peri_out,pkperiods,pkheights
  pkh=max(pkheights)
  ...
  for j=0,ntrial-1 do begin
	  for jj=0,ndata-1 do begin    ;replacement with redraw
		 ...
		 cf3[jj].mnvel=cf3[dum].mnvel-mnvel
	  endfor
	  pergram,cf3,nu_out,peri_out,pkperiods,pkheights
	  if j mod 10 then begin
	  dum = where(mc_arr gt pkh, fas)
	  print, 'Percentage Complete: ', j/ntrial, ' %.', $
	  ' FA so far: ', fas
	  endif
...
end;mpf_widget_fap.pro

pro mpf_widget_fapiter, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fapiter.pro

pro mpf_widget_run, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
print, 'rms is: ', rms
 ...
 ;the uncertainties:
 mpf_widget_retrieve_par, pstate, pararr, perror

 print, 'COMPLETED MPFITFUN AND SENT TO MPF_WIDGET_DOFIT'

  ;Update the text fields with the new parameters:
  mpf_widget_update_fields, pstate
  
end ;mpf_widget_run.pro

pro mpf_widget_cyclerun, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
  widget_control, event.id, get_value=oldway
...
end ;mpf_widget_cyclerun.pro

pro mpf_widget_rvlin, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
  widget_control, event.id, get_value=oldway
...
pararr[5*indx+3].value = orbel[7*indx+3]
k = orbel[7*indx+4]
pararr[5*n_planets].value = orbel[5]
pararr[5*n_planets+1].value = orbel[6]
...
end ;mpf_widget_rvlin.pro

pro mpf_widget_monte, event
...
end;mpf_widget_monte.pro

pro mpf_widget_monte2, event
...
if n_planets gt 1 then eccmax[1] = pararr[7].limits[1]
;stop
...
if offbyper gt 0 then t_center[x] += period
...
print,'gam: '+mediangam+' +/- '+gamerr
print,'dvdt: '+mediandvdt+' +/- '+dvdterr
print,'msini: ', medianmass+' +/- '+masserr
print,'arel: ', medianarel+' +/- '+arelerr
print,'t_center: ', mediantcen+' +/- '+tcenerr
print,'t_duration: ', mediantdur+' +/- '+tdurerr
print, '' & print, ''
...
stop
end;mpf_widget_monte2.pro

pro mpf_widget_saveresid, event
...
end;mpf_widget_saveresid.pro

pro mpf_widget_restoreresid, event
...
end;mpf_widget_restoreresid.pro

pro mpf_widget_plotresid, event
 ...
 widget_control, event.top, get_uvalue=pstate
  ...
  plot, fitobs, fitdat, $
	 linestyle=(*pstate).linestyle, $
	 color = 0, psym=(*pstate).psym*(*pstate).connect, $
	 XTickFormat='mpf_rvdatelabel', xminor=4, XTICKINTERVAL = rvran/7, $
	 xtitle = 'Time of Observation', $
...
end ;mpf_widget_plotresid.pro

pro mpf_widget_plotps, event
...
end ;mpf_widget_plotps.pro

pro mpf_widget_sliderzoom, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end ;mpf_widget_sliderzoom.pro

pro mpf_widget_sliderscroll, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end ;mpf_widget_sliderscroll.pro

pro mpf_widget_xminxmax, pstate
...
end ;mpf_widget_xminmax.pro

pro mpf_widget_zoomrv, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
  ...
  ;Call the "scroll slider" routine to updat xmin and xmax:
  mpf_widget_dofit, pstate
end ;mpf_widget_zoomrv.pro



pro mpf_widget_multith, event
  ;This procedure will tie m & i for the case where the radial velocity
  ; data looks really good and you want to adjust the astrometry. 
...
end;mpf_widget_multith.pro

pro mpf_widget_togerr, event
  ;This procedure will tie m & i for the case where the radial velocity
  ; data looks really good and you want to adjust the astrometry. 
...
end;mpf_widget_togerr.pro


pro mpf_widget_connect, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_connect.pro

pro mpf_widget_printpars, event
  ...
  mpf_widget_external_texpars, pstate
  
  ;Call the "do" routine:
  mpf_widget_dofit, pstate

end;mpf_widget_printpars.pro

pro mpf_widget_printplpars, event
  ...
  ;Flip the sign of the connect field:
  ;(*pstate).printpars = event.select
  mpf_widget_external_texplpars, pstate
  
  ;Call the "do" routine:
  mpf_widget_dofit, pstate

end;mpf_widget_printplpars.pro

pro mpf_widget_tfinebttn, event
...
end;mpf_widget_tfinebttn.pro

pro mpf_widget_sym, event
...
end;mpf_widget_sym.pro

pro mpf_widget_ls, event
...
end;mpf_widget_ls.pro

pro mpf_widget_plotorbit, event
...
end;mpf_widget_plotorbit.pro

pro mpf_widget_addjitter, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_addjitter.pro

pro mpf_widget_jitternum, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
  ...
  ;stop
end;mpf_widget_jitternum.pro


pro mpf_widget_debug, event
 ; Get the pointer to the state structure from the user value
 ; of the top-level base.
 widget_control, event.top, get_uvalue=pstate
...
pro mpf_widget_fitplnum, event
 ...
 end ;mpf_widget_fix1.pro

 pro mpf_widget_fix2, event
   ;Retrieve the pointer to the state structure:
   widget_control, event.top, get_uvalue=pstate
 ...
 end ;mpf_widget_fix2.pro

 pro mpf_widget_fix3, event
   ;Retrieve the pointer to the state structure:
   widget_control, event.top, get_uvalue=pstate
   ...
   print, 'Planet 2 Fixed? ', push
 end ;mpf_widget_fix3.pro

 pro mpf_widget_fix4, event
   ;Retrieve the pointer to the state structure:
   widget_control, event.top, get_uvalue=pstate
 ...
 end ;mpf_widget_fix4.pro

 pro mpf_widget_fix5, event
   ;Retrieve the pointer to the state structure:
   widget_control, event.top, get_uvalue=pstate
 ...
 end ;mpf_widget_fix5.pro

 pro mpf_widget_fix6, event
   ;Retrieve the pointer to the state structure:
   widget_control, event.top, get_uvalue=pstate
 ...
 end ;mpf_widget_fix6.pro

 pro mpf_widget_fix7, event
   ;Retrieve the pointer to the state structure:
   widget_control, event.top, get_uvalue=pstate
 ...
 ;**************************************************************
 ;**************************************************************
pro mpf_widget_texperpl1, event
  ;Retrieve the pointer to the state structure:
...
end;mpf_widget_texperpl1.pro

pro mpf_widget_texperypl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newper
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texperypl1.pro

pro mpf_widget_texmasspl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texmasspl1.pro

pro mpf_widget_texeccpl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texeccpl1.pro

pro mpf_widget_texincpl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texincpl1.pro

pro mpf_widget_texbompl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texbompl1.pro

pro mpf_widget_texlompl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texlompl1.pro

pro mpf_widget_textoppl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
 ...
 ;**************************************************************
 ;**************************************************************
 pro mpf_widget_fixperpl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixperpl1.pro
 
 pro mpf_widget_fixmasspl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixmasspl1.pro
 
 pro mpf_widget_fixeccpl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixeccpl1.pro
 
 pro mpf_widget_fixincpl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixincpl1.pro
 
 pro mpf_widget_fixbompl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixbompl1.pro
 
 pro mpf_widget_fixlompl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixlompl1.pro
 
 pro mpf_widget_fixtoppl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
pro mpf_widget_sliperpl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_sliperpl1.pro

pro mpf_widget_slimasspl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slimasspl1.pro

pro mpf_widget_slieccpl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slieccpl1.pro

pro mpf_widget_sliincpl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_sliincpl1.pro

pro mpf_widget_slibompl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slibompl1.pro

pro mpf_widget_slilompl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slilompl1.pro

pro mpf_widget_slitoppl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
  top = event.value
 ...
 ;**************************************************************
 ;**************************************************************
pro mpf_widget_perlowlimpl1, event
  ;Retrieve the pointer to the state structure:
...
end;mpf_widget_perlowlimpl1.pro

pro mpf_widget_perhilimpl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_perhilimpl1.pro

pro mpf_widget_masslowlimpl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_masslowlimpl1.pro

pro mpf_widget_masshilimpl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_masshilimpl1.pro

pro mpf_widget_ecclowlimpl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_ecclowlimpl1.pro

pro mpf_widget_ecchilimpl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_ecchilimpl1.pro

pro mpf_widget_inclowlimpl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_inclowlimpl1.pro

pro mpf_widget_inchilimpl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_inchilimpl1.pro

pro mpf_widget_bomlowlimpl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_bomlowlimpl1.pro

pro mpf_widget_bomhilimpl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_bomhilimpl1.pro

pro mpf_widget_lomlowlimpl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_lomlowlimpl1.pro

pro mpf_widget_lomhilimpl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_lomhilimpl1.pro

pro mpf_widget_toplowlimpl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_toplowlimpl1.pro

pro mpf_widget_tophilimpl1, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_tophilimpl1.pro
 ...
 ;**************************************************************
 ;**************************************************************
pro mpf_widget_texperpl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newper
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texperpl2.pro

pro mpf_widget_texperypl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newper
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texperypl2.pro

pro mpf_widget_texmasspl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texmasspl2.pro


pro mpf_widget_texeccpl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texeccpl2.pro

pro mpf_widget_texincpl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texincpl2.pro

pro mpf_widget_texbompl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texbompl2.pro

pro mpf_widget_texlompl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texlompl2.pro

pro mpf_widget_textoppl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
 ...
 ;**************************************************************
 ;**************************************************************
 pro mpf_widget_fixperpl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixperpl2.pro
 
 pro mpf_widget_fixmasspl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixmasspl2.pro
 
 pro mpf_widget_fixeccpl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixeccpl2.pro
 
 pro mpf_widget_fixincpl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixincpl2.pro
 
 pro mpf_widget_fixbompl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixbompl2.pro
 
 pro mpf_widget_fixlompl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixlompl2.pro
 
 pro mpf_widget_fixtoppl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
pro mpf_widget_sliperpl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_sliperpl2.pro

pro mpf_widget_slimasspl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slimasspl2.pro

pro mpf_widget_slieccpl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slieccpl2.pro

pro mpf_widget_sliincpl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_sliincpl2.pro

pro mpf_widget_slibompl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slibompl2.pro

pro mpf_widget_slilompl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slilompl2.pro

pro mpf_widget_slitoppl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
  top = event.value
 ...
 ;**************************************************************
 ;**************************************************************
pro mpf_widget_perlowlimpl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_perlowlimpl2.pro

pro mpf_widget_perhilimpl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_perhilimpl2.pro

pro mpf_widget_masslowlimpl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_masslowlimpl2.pro

pro mpf_widget_masshilimpl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_masshilimpl2.pro

pro mpf_widget_ecclowlimpl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_ecclowlimpl2.pro

pro mpf_widget_ecchilimpl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_ecchilimpl2.pro

pro mpf_widget_inclowlimpl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_inclowlimpl2.pro

pro mpf_widget_inchilimpl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_inchilimpl2.pro

pro mpf_widget_bomlowlimpl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_bomlowlimpl2.pro

pro mpf_widget_bomhilimpl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_bomhilimpl2.pro

pro mpf_widget_lomlowlimpl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_lomlowlimpl2.pro

pro mpf_widget_lomhilimpl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_lomhilimpl2.pro

pro mpf_widget_toplowlimpl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_toplowlimpl2.pro

pro mpf_widget_tophilimpl2, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_tophilimpl2.pro
 ...
 ;**************************************************************
 ;**************************************************************
pro mpf_widget_texperpl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newper
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texperpl3.pro

pro mpf_widget_texperypl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newper
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texperypl3.pro

pro mpf_widget_texmasspl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texmasspl3.pro

pro mpf_widget_texeccpl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texeccpl3.pro

pro mpf_widget_texincpl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texincpl3.pro

pro mpf_widget_texbompl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texbompl3.pro

pro mpf_widget_texlompl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texlompl3.pro

pro mpf_widget_textoppl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
 ...
 ;**************************************************************
 ;**************************************************************
 pro mpf_widget_fixperpl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixperpl3.pro
 
 pro mpf_widget_fixmasspl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixmasspl3.pro
 
 pro mpf_widget_fixeccpl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixeccpl3.pro
 
 pro mpf_widget_fixincpl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixincpl3.pro
 
 pro mpf_widget_fixbompl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixbompl3.pro
 
 pro mpf_widget_fixlompl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixlompl3.pro
 
 pro mpf_widget_fixtoppl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
pro mpf_widget_sliperpl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_sliperpl3.pro

pro mpf_widget_slimasspl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slimasspl3.pro

pro mpf_widget_slieccpl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slieccpl3.pro

pro mpf_widget_sliincpl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_sliincpl3.pro

pro mpf_widget_slibompl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slibompl3.pro

pro mpf_widget_slilompl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slilompl3.pro

pro mpf_widget_slitoppl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
  top = event.value
 ...
 ;**************************************************************
 ;**************************************************************
pro mpf_widget_perlowlimpl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_perlowlimpl3.pro

pro mpf_widget_perhilimpl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_perhilimpl3.pro

pro mpf_widget_masslowlimpl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_masslowlimpl3.pro

pro mpf_widget_masshilimpl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_masshilimpl3.pro

pro mpf_widget_ecclowlimpl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_ecclowlimpl3.pro

pro mpf_widget_ecchilimpl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_ecchilimpl3.pro

pro mpf_widget_inclowlimpl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_inclowlimpl3.pro

pro mpf_widget_inchilimpl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_inchilimpl3.pro

pro mpf_widget_bomlowlimpl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_bomlowlimpl3.pro

pro mpf_widget_bomhilimpl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_bomhilimpl3.pro

pro mpf_widget_lomlowlimpl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_lomlowlimpl3.pro

pro mpf_widget_lomhilimpl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_lomhilimpl3.pro

pro mpf_widget_toplowlimpl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_toplowlimpl3.pro

pro mpf_widget_tophilimpl3, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_tophilimpl3.pro
 ...
 ;**************************************************************
 ;**************************************************************
pro mpf_widget_texperpl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newper
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texperpl4.pro

pro mpf_widget_texperypl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newper
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texperypl4.pro

pro mpf_widget_texmasspl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texmasspl4.pro

pro mpf_widget_texeccpl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texeccpl4.pro

pro mpf_widget_texincpl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texincpl4.pro

pro mpf_widget_texbompl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texbompl4.pro

pro mpf_widget_texlompl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texlompl4.pro

pro mpf_widget_textoppl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
 ...
 ;**************************************************************
 ;**************************************************************
 pro mpf_widget_fixperpl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixperpl4.pro
 
 pro mpf_widget_fixmasspl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixmasspl4.pro
 
 pro mpf_widget_fixeccpl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixeccpl4.pro
 
 pro mpf_widget_fixincpl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixincpl4.pro
 
 pro mpf_widget_fixbompl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixbompl4.pro
 
 pro mpf_widget_fixlompl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixlompl4.pro
 
 pro mpf_widget_fixtoppl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
pro mpf_widget_sliperpl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_sliperpl4.pro

pro mpf_widget_slimasspl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slimasspl4.pro

pro mpf_widget_slieccpl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slieccpl4.pro

pro mpf_widget_sliincpl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_sliincpl4.pro

pro mpf_widget_slibompl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slibompl4.pro

pro mpf_widget_slilompl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slilompl4.pro

pro mpf_widget_slitoppl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
  top = event.value
 ...
 ;**************************************************************
 ;**************************************************************
pro mpf_widget_perlowlimpl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_perlowlimpl4.pro

pro mpf_widget_perhilimpl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_perhilimpl4.pro

pro mpf_widget_masslowlimpl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_masslowlimpl4.pro

pro mpf_widget_masshilimpl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_masshilimpl4.pro

pro mpf_widget_ecclowlimpl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_ecclowlimpl4.pro

pro mpf_widget_ecchilimpl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_ecchilimpl4.pro

pro mpf_widget_inclowlimpl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_inclowlimpl4.pro

pro mpf_widget_inchilimpl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_inchilimpl4.pro

pro mpf_widget_bomlowlimpl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_bomlowlimpl4.pro

pro mpf_widget_bomhilimpl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_bomhilimpl4.pro

pro mpf_widget_lomlowlimpl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_lomlowlimpl4.pro

pro mpf_widget_lomhilimpl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_lomhilimpl4.pro

pro mpf_widget_toplowlimpl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_toplowlimpl4.pro

pro mpf_widget_tophilimpl4, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_tophilimpl4.pro
 ...
 ;**************************************************************
 ;**************************************************************
pro mpf_widget_texperpl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newper
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texperpl5.pro

pro mpf_widget_texperypl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newper
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texperypl5.pro

pro mpf_widget_texmasspl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texmasspl5.pro

pro mpf_widget_texeccpl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texeccpl5.pro

pro mpf_widget_texincpl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texincpl5.pro

pro mpf_widget_texbompl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texbompl5.pro

pro mpf_widget_texlompl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texlompl5.pro

pro mpf_widget_textoppl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
 ...
 ;**************************************************************
 ;**************************************************************
 pro mpf_widget_fixperpl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixperpl5.pro
 
 pro mpf_widget_fixmasspl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixmasspl5.pro
 
 pro mpf_widget_fixeccpl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixeccpl5.pro
 
 pro mpf_widget_fixincpl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixincpl5.pro
 
 pro mpf_widget_fixbompl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixbompl5.pro
 
 pro mpf_widget_fixlompl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixlompl5.pro
 
 pro mpf_widget_fixtoppl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
pro mpf_widget_sliperpl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_sliperpl5.pro

pro mpf_widget_slimasspl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slimasspl5.pro

pro mpf_widget_slieccpl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slieccpl5.pro

pro mpf_widget_sliincpl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_sliincpl5.pro

pro mpf_widget_slibompl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slibompl5.pro

pro mpf_widget_slilompl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slilompl5.pro

pro mpf_widget_slitoppl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
  top = event.value
 ...
 ;**************************************************************
 ;**************************************************************
pro mpf_widget_perlowlimpl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_perlowlimpl5.pro

pro mpf_widget_perhilimpl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_perhilimpl5.pro

pro mpf_widget_masslowlimpl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_masslowlimpl5.pro

pro mpf_widget_masshilimpl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_masshilimpl5.pro

pro mpf_widget_ecclowlimpl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_ecclowlimpl5.pro

pro mpf_widget_ecchilimpl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_ecchilimpl5.pro

pro mpf_widget_inclowlimpl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_inclowlimpl5.pro

pro mpf_widget_inchilimpl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_inchilimpl5.pro

pro mpf_widget_bomlowlimpl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_bomlowlimpl5.pro

pro mpf_widget_bomhilimpl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_bomhilimpl5.pro

pro mpf_widget_lomlowlimpl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_lomlowlimpl5.pro

pro mpf_widget_lomhilimpl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_lomhilimpl5.pro

pro mpf_widget_toplowlimpl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_toplowlimpl5.pro

pro mpf_widget_tophilimpl5, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_tophilimpl5.pro
 ...
 ;**************************************************************
 ;**************************************************************
pro mpf_widget_texperpl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newper
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texperpl6.pro

pro mpf_widget_texperypl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newper
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texperypl6.pro

pro mpf_widget_texmasspl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texmasspl6.pro

pro mpf_widget_texeccpl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texeccpl6.pro

pro mpf_widget_texincpl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texincpl6.pro

pro mpf_widget_texbompl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texbompl6.pro

pro mpf_widget_texlompl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texlompl6.pro

pro mpf_widget_textoppl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
 ...
 ;**************************************************************
 ;**************************************************************
 pro mpf_widget_fixperpl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixperpl6.pro
 
 pro mpf_widget_fixmasspl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixmasspl6.pro
 
 pro mpf_widget_fixeccpl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixeccpl6.pro
 
 pro mpf_widget_fixincpl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixincpl6.pro
 
 pro mpf_widget_fixbompl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixbompl6.pro
 
 pro mpf_widget_fixlompl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixlompl6.pro
 
 pro mpf_widget_fixtoppl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
pro mpf_widget_sliperpl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_sliperpl6.pro

pro mpf_widget_slimasspl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slimasspl6.pro

pro mpf_widget_slieccpl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slieccpl6.pro

pro mpf_widget_sliincpl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_sliincpl6.pro

pro mpf_widget_slibompl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slibompl6.pro

pro mpf_widget_slilompl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slilompl6.pro

pro mpf_widget_slitoppl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
  top = event.value
 ...
 ;**************************************************************
 ;**************************************************************
pro mpf_widget_perlowlimpl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_perlowlimpl6.pro

pro mpf_widget_perhilimpl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_perhilimpl6.pro

pro mpf_widget_masslowlimpl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_masslowlimpl6.pro

pro mpf_widget_masshilimpl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_masshilimpl6.pro

pro mpf_widget_ecclowlimpl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_ecclowlimpl6.pro

pro mpf_widget_ecchilimpl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_ecchilimpl6.pro

pro mpf_widget_inclowlimpl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_inclowlimpl6.pro

pro mpf_widget_inchilimpl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_inchilimpl6.pro

pro mpf_widget_bomlowlimpl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_bomlowlimpl6.pro

pro mpf_widget_bomhilimpl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_bomhilimpl6.pro

pro mpf_widget_lomlowlimpl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_lomlowlimpl6.pro

pro mpf_widget_lomhilimpl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_lomhilimpl6.pro

pro mpf_widget_toplowlimpl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_toplowlimpl6.pro

pro mpf_widget_tophilimpl6, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_tophilimpl6.pro
 ...
 ;**************************************************************
 ;**************************************************************
pro mpf_widget_texperpl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newper
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texperpl7.pro

pro mpf_widget_texperypl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newper
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texperypl7.pro

pro mpf_widget_texmasspl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texmasspl7.pro

pro mpf_widget_texeccpl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texeccpl7.pro

pro mpf_widget_texincpl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texincpl7.pro

pro mpf_widget_texbompl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texbompl7.pro

pro mpf_widget_texlompl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texlompl7.pro

pro mpf_widget_textoppl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
 ...
 ;**************************************************************
 ;**************************************************************
 pro mpf_widget_fixperpl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixperpl7.pro
 
 pro mpf_widget_fixmasspl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixmasspl7.pro
 
 pro mpf_widget_fixeccpl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixeccpl7.pro
 
 pro mpf_widget_fixincpl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixincpl7.pro
 
 pro mpf_widget_fixbompl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixbompl7.pro
 
 pro mpf_widget_fixlompl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixlompl7.pro
 
 pro mpf_widget_fixtoppl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
pro mpf_widget_sliperpl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_sliperpl7.pro

pro mpf_widget_slimasspl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slimasspl7.pro

pro mpf_widget_slieccpl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slieccpl7.pro

pro mpf_widget_sliincpl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_sliincpl7.pro

pro mpf_widget_slibompl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slibompl7.pro

pro mpf_widget_slilompl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slilompl7.pro

pro mpf_widget_slitoppl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
  top = event.value
 ...
 ;**************************************************************
 ;**************************************************************
pro mpf_widget_perlowlimpl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_perlowlimpl7.pro

pro mpf_widget_perhilimpl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_perhilimpl7.pro

pro mpf_widget_masslowlimpl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_masslowlimpl7.pro

pro mpf_widget_masshilimpl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_masshilimpl7.pro

pro mpf_widget_ecclowlimpl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_ecclowlimpl7.pro

pro mpf_widget_ecchilimpl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_ecchilimpl7.pro

pro mpf_widget_inclowlimpl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_inclowlimpl7.pro

pro mpf_widget_inchilimpl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_inchilimpl7.pro

pro mpf_widget_bomlowlimpl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_bomlowlimpl7.pro

pro mpf_widget_bomhilimpl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_bomhilimpl7.pro

pro mpf_widget_lomlowlimpl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_lomlowlimpl7.pro

pro mpf_widget_lomhilimpl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_lomhilimpl7.pro

pro mpf_widget_toplowlimpl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_toplowlimpl7.pro

pro mpf_widget_tophilimpl7, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_tophilimpl7.pro
...
end;mpf_widget_texrvo.pro

pro mpf_widget_texdew24, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texdew24.pro

pro mpf_widget_texdew39, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texdew39.pro

pro mpf_widget_texsrv, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_texsrv.pro

pro mpf_widget_texcrv, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newpar
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixrvo.pro
 
pro mpf_widget_fixsrv, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixsrv.pro
 
 pro mpf_widget_fixcrv, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixcrv.pro
 
 pro mpf_widget_fixdew24, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_fixdew24.pro
 
 pro mpf_widget_fixdew39, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slirvo.pro

pro mpf_widget_slisrv, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slisrv.pro

pro mpf_widget_slicrv, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slicrv.pro

pro mpf_widget_slidew24, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_slidew24.pro

pro mpf_widget_slidew39, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.top, get_uvalue=pstate
...
pro mpf_widget_rvolowlim, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_rvolowlim.pro

pro mpf_widget_rvohilim, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_rvohilim.pro

pro mpf_widget_srvlowlim, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_srvlowlim.pro

pro mpf_widget_srvhilim, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_srvhilim.pro

pro mpf_widget_crvlowlim, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
end;mpf_widget_crvlowlim.pro

pro mpf_widget_crvhilim, event
  ;Retrieve the pointer to the state structure:
  widget_control, event.id, get_value=newlim
  widget_control, event.top, get_uvalue=pstate
...
pro kfme
 ...
 cfname = 'cf1_001bl21'
 restore, '/mir1/mpflib/data/datstructures/'+cfname+'.dat'
 
 cfname = '/mir1/vel/vst9826.dat'
 restore, cfname
...
;endif
 ...
 tlb = widget_base(title = 'Interactive RV v. 2010/12/09 ', $
 ...
 event_pro = 'mpf_widget_openlickvst',xsize=halfcol)
 
 openvst = widget_button(telrow, value = 'OPEN KECK', $
 event_pro = 'mpf_widget_openkeckvst',xsize=halfcol)
 
 orbitrow = widget_base(controlbase, /row)
 savresid = widget_button(orbitrow, value = 'PLOT ORBIT', $
 event_pro = 'mpf_widget_plotorbit', XSIZE=halfcol)
 
 savrow = widget_base(controlbase, /row)

 savresid = widget_button(savrow, value = 'SAVE ALL', $
 event_pro = 'mpf_widget_saveall', xsize=halfcol)
 
 residbuttn = widget_button(savrow, value = 'RESTORE ALL', $
 event_pro = 'mpf_widget_restoreall', xsize=halfcol)
 ...
 plotdatabuttn = widget_button(datrow, value = 'DATA', $
	 event_pro = 'mpf_widget_plotdata', xsize=halfcol)
  
 plotresidbuttn = widget_button(datrow, value = 'RESID DATA', $
 event_pro = 'mpf_widget_plotresid', xsize=halfcol)
 
 alonerow = widget_base(controlbase, /row)
 
 aloneplanet = 2
 plalnumbox = widget_text(alonerow, value = strt(aloneplanet), $
 	/editable, event_pro = 'mpf_widget_planetalone', xsize = '3')

 alonebuttn = widget_button(alonerow, value = 'REMOVE OTHERS', $
 event_pro = 'mpf_widget_aloneplot', xsize=halfcol)
 ...
 phasebutton = widget_button(phasebase, $
   value = 'PHASE', $
   event_pro = 'mpf_widget_orbitphase', xsize=halfcol)
 ...
 lowpergval = widget_text(lowperg, value = strt(0.75), $
 	/editable, event_pro = 'mpf_widget_lowperg', xsize = '15')
 ...
 hipergval = widget_text(hiperg, value = strt(10000), $
 	/editable, event_pro = 'mpf_widget_hiperg', xsize = '15')
 ...
 pergresval = widget_text(pergresbox, value = strt(pergres), $
 	/editable, event_pro = 'mpf_widget_pergres', xsize = '15')

 faprow = widget_base(controlbase, /row)

 fapbuttn = widget_button(faprow, value = 'FAP', $
   event_pro = 'mpf_widget_fap', xsize=halfcol)
   
 fapiter = 1000.
 fapval = widget_text(faprow, value = strt(fapiter), $
 	/editable, event_pro = 'mpf_widget_fapiter', xsize = halfcol)

 
 runrow = widget_base(controlbase, /row)
 runbuttn = widget_button(runrow, value = 'RUN...', $
   event_pro = 'mpf_widget_run', XSIZE=HALFCOL)
 
 runbuttn = widget_button(runrow, value = 'CYCLE RUN...', $
   event_pro = 'mpf_widget_cyclerun', XSIZE=halfcol)
 
 run2row = widget_base(controlbase, /row)
 runbuttn = widget_button(run2row, value = 'RVLIN', $
   event_pro = 'mpf_widget_rvlin', XSIZE=thirdcol)
 montebuttn = widget_button(run2row, value = 'MONTE', $
   event_pro = 'mpf_widget_monte', XSIZE=thirdcol)
 monte2buttn = widget_button(run2row, value = 'MNTE2', $
   event_pro = 'mpf_widget_monte2', XSIZE=thirdcol)
 
 residrow = widget_base(controlbase, /row)
 savresid = widget_button(residrow, value = 'SAVE RESID', $
 event_pro = 'mpf_widget_saveresid', XSIZE=halfcol)
 
 residbuttn = widget_button(residrow, value = 'RESTORE RESID', $
 event_pro = 'mpf_widget_restoreresid', XSIZE=halfcol)
 ...
 psplotbutton = widget_button(psplotbase, $
   value = 'PS', $
   event_pro = 'mpf_widget_plotps', XSIZE=thirdcol)
 ...
 multithbutton = widget_button(multithbase, $
   value = 'MULTI', $
   event_pro = 'mpf_widget_multith', xsize=thirdcol)
...
tfinebutton = widget_button(tfinebase, $
 ...
 zoomslide = widget_slider(controlbase, title = 'Zoom', /suppress_val, $
    ...
    event_pro = 'mpf_widget_sliderzoom', /drag)
 
 ;The slider to Scroll:
 scrollslide = widget_slider(controlbase, title='Scroll', $
 	/suppress_val, $
    value = 0, minimum = -100., maximum = 100., $
    event_pro = 'mpf_widget_sliderscroll', /drag)
 
 jitterrow = widget_base(controlbase, /row)

 jitterbuttn = widget_button(jitterrow, value = 'jitter', $
   event_pro = 'mpf_widget_addjitter', xsize=halfcol)
   
 jitternum = 0d
 previousjitter = 0d
 jitterval = widget_text(jitterrow, value = strt(jitternum), $
 	/editable, event_pro = 'mpf_widget_jitternum', xsize = halfcol)
 ...
 title = '', event_pro = 'mpf_widget_ls', xsize=halfcol)
 
;PLOTORBIT GOES HERE


 plotresidbuttn = widget_button(controlbase, value = 'DEBUG', $
 event_pro = 'mpf_widget_debug')
 ...
 fitpar1bttn = widget_button(fitplanetbase, value = 'FIT PLANET 1 ', $
        event_pro = 'mpf_widget_fitplnum')
 
 widget_control, fitpar1bttn, /set_button

 resetbtn1 = widget_button(planetcontrolbase,value = 'RESET PARAMETERS',event_pro = 'mpf_widget_resetparam', xsize = 120)
 ;***************TABSTUFF************************************
 ...
 pertextpl1 = widget_text(pernums, value = strt(initper), $
 	/editable, event_pro = 'mpf_widget_texperpl1', xsize = 10)

 pertextypl1 = widget_text(pernums, value = strt(initper/365.2564d), $
 	/editable, event_pro = 'mpf_widget_texperypl1', xsize = 10)

 sliderperpl1 = widget_slider(orbpar1, event_pro = $
 	'mpf_widget_sliperpl1', /suppress_value, /drag, $
 ...
 periodlowlimpl1 = widget_text(lowlim, value = $
 	...
 	'mpf_widget_perlowlimpl1', xsize = '12')
 ...
 periodhilimpl1 = widget_text(uplim, value = strt(par1[0].limits[1]), $
 	/editable, event_pro = 'mpf_widget_perhilimpl1', xsize = '12')
...
amptitle = widget_base(orbpar2, /row)
 ...
 event_pro = 'mpf_widget_texamppl1', xsize = 10) ;, /all_events

 slidermasspl1 = widget_slider(orbpar2, event_pro = $
 	'mpf_widget_slimasspl1', /suppress_value, /drag, $
 ...
 lowlimvalpl1 = widget_text(lowlim, value = strt(par1[1].limits[0]), $
   /editable, event_pro = 'mpf_widget_masslowlimpl1')
 ...
 uplimvalpl1 = widget_text(uplim, value = strt(par1[1].limits[1]), $
 	/editable, event_pro = 'mpf_widget_masshilimpl1')
 ...
 /editable, $
 event_pro = 'mpf_widget_ecclowlimpl1', xsize = '12')
 ...
 /editable, event_pro = 'mpf_widget_ecchilimpl1', xsize = '12')
 ...
 lowlimvalpl1 = widget_text(lowlim, value = strt(par1[3].limits[0]), $
 	/editable, event_pro = 'mpf_widget_lomlowlimpl1', xsize = '12')
 ...
 uplimvalpl1 = widget_text(uplim, value = '360', /editable, $
 	event_pro = 'mpf_widget_lomhilimpl1', xsize = '12')
 ...
 textpar = widget_text(orbpar5, value = 'T_0 RV [d]', $
 	event_pro = 'mpf_widget_textoppl1', xsize = 10)
 
 fixpar = widget_base(orbpar5, /row)
 
 toptextpl1 = widget_text(fixpar, value = strt(inittop), $
 	/editable, event_pro = 'mpf_widget_textoppl1', xsize = 10)

 radiobase = widget_base(fixpar, /nonexclusive)

 fixtoppl1 = widget_button(radiobase, value = 'fix', $
 	event_pro = 'mpf_widget_fixtoppl1')
 
 slidertoppl1 = widget_slider(orbpar5, $
 	event_pro = 'mpf_widget_slitoppl1', /suppress_value, $
 ...
 lowlimvalpl1 = widget_text(lowlim, value = '1.00', /editable, $
 	event_pro = 'mpf_widget_toplowlimpl1', xsize = '12')
 ...
 uplimvalpl1 = widget_text(uplim, value = '10000', /editable, $
 	event_pro = 'mpf_widget_tophilimpl1', xsize = '12')
...
;***************TABSTUFF************************************
 ...
 resetbtn2 = widget_button(planetcontrolbase,value = 'RESET PARAMETERS',event_pro = 'mpf_widget_resetparam', xsize = 120)
 ;***************TABSTUFF************************************
 ...
 fixperpl2 = widget_button(radiobase, value = 'fix', $
 	event_pro = 'mpf_widget_fixperpl2')
 ...
 pertextpl2 = widget_text(pernums, value = strt(initper), $
 	/editable, event_pro = 'mpf_widget_texperpl2', xsize = 10)

 pertextypl2 = widget_text(pernums, value = strt(initper/365.2564d), $
 	/editable, event_pro = 'mpf_widget_texperypl2', xsize = 10)

 sliderperpl2 = widget_slider(orbpar2, event_pro = $
 	'mpf_widget_sliperpl2', /suppress_value, /drag, $
 	minimum = 1, maximum = 30000., value = initper)
 ...
 periodlowlimpl2 = widget_text(lowlim, value = $
 	...
 	'mpf_widget_perlowlimpl2', xsize = '12')
 ...
 periodhilimpl2 = widget_text(uplim, value = strt(par2[0].limits[1]), $
 	/editable, event_pro = 'mpf_widget_perhilimpl2', xsize = '12')
 ...
 event_pro = 'mpf_widget_texmasspl2', xsize = 10) ;, /all_events

 radiobase = widget_base(fixpar, /nonexclusive)
 fixmasspl2 = widget_button(radiobase, value = 'fix', $
 event_pro = 'mpf_widget_fixmasspl2')
 
 slidermasspl2 = widget_slider(orbpar2, event_pro = $
 	'mpf_widget_slimasspl2', /suppress_value, /drag, $
 	minimum = 1, maximum = 5d5, value = initmass*100.)
 ...
 /editable, $
 event_pro = 'mpf_widget_masslowlimpl2', xsize = '12')
 ...
 uplimvalpl2 = widget_text(uplim, value = strt(par2[1].limits[1]), $
 	/editable, event_pro = 'mpf_widget_masshilimpl2', xsize = '12')
 ...
 /editable, $
 event_pro = 'mpf_widget_texeccpl2', xsize = 10)

 radiobase = widget_base(fixpar, /nonexclusive)
 fixeccpl2 = widget_button(radiobase, value = 'fix', $
 event_pro = 'mpf_widget_fixeccpl2')
 
 slidereccpl2 = widget_slider(orbpar3, event_pro = $
 	'mpf_widget_slieccpl2', /suppress_value, /drag, $
 	minimum = 1.d, maximum = 999.d, value = initecc*1000.)
 ...
 /editable, $
 event_pro = 'mpf_widget_ecclowlimpl2', xsize = '12')
 ...
 /editable, event_pro = 'mpf_widget_ecchilimpl2', xsize = '12')
 ...
 lomtextpl2 = widget_text(fixpar, value = strt(initlom), $
 	/editable, event_pro = 'mpf_widget_texlompl2', xsize = 10)

 radiobase = widget_base(fixpar, /nonexclusive)

 fixlompl2 = widget_button(radiobase, value = 'fix', $
 	event_pro = 'mpf_widget_fixlompl2')
 
 sliderlompl2 = widget_slider(orbpar6, event_pro = $
 	'mpf_widget_slilompl2', /suppress_value, /drag, $
 	minimum = 0.0, maximum = 360., value = initlom)
 ...
 lowlimvalpl2 = widget_text(lowlim, value = strt(par2[3].limits[0]), $
 	/editable, event_pro = 'mpf_widget_lomlowlimpl2', xsize = '12')
 ...
 uplimvalpl2 = widget_text(uplim, value = '360', /editable, $
 	event_pro = 'mpf_widget_lomhilimpl2', xsize = '12')
 ...
 textpar = widget_text(orbpar6, value = 'T_0 RV [d]', $
 	event_pro = 'mpf_widget_textoppl2', xsize = 10)
 
 fixpar = widget_base(orbpar6, /row)
 
 toptextpl2 = widget_text(fixpar, value = strt(inittop), $
 	/editable, event_pro = 'mpf_widget_textoppl2', xsize = 10)

 radiobase = widget_base(fixpar, /nonexclusive)

 fixtoppl2 = widget_button(radiobase, value = 'fix', $
 	event_pro = 'mpf_widget_fixtoppl2')
 
 slidertoppl2 = widget_slider(orbpar6, $
 	event_pro = 'mpf_widget_slitoppl2', /suppress_value, $
 ...
 lowlimvalpl2 = widget_text(lowlim, value = '1.00', /editable, $
 	event_pro = 'mpf_widget_toplowlimpl2', xsize = '12')
 ...
 uplimvalpl2 = widget_text(uplim, value = '10000', /editable, $
 	event_pro = 'mpf_widget_tophilimpl2', xsize = '12')
...
;***************TABSTUFF************************************
 ...
 resetbtn3 = widget_button(planetcontrolbase,value = 'RESET PARAMETERS',event_pro = 'mpf_widget_resetparam', xsize = 120)
 ;***************TABSTUFF************************************
 ...
 fixperpl3 = widget_button(radiobase, value = 'fix', $
 	event_pro = 'mpf_widget_fixperpl3')
 ...
 pertextpl3 = widget_text(pernums, value = strt(initper), $
 	/editable, event_pro = 'mpf_widget_texperpl3', xsize = 10)

 pertextypl3 = widget_text(pernums, value = strt(initper/365.2564d), $
 	/editable, event_pro = 'mpf_widget_texperypl3', xsize = 10)

 sliderperpl3 = widget_slider(orbpar3, event_pro = $
 	'mpf_widget_sliperpl3', /suppress_value, /drag, $
 	minimum = 1, maximum = 30000., value = initper)
 ...
 periodlowlimpl3 = widget_text(lowlim, value = $
 	...
 	'mpf_widget_perlowlimpl3', xsize = '12')
 ...
 periodhilimpl3 = widget_text(uplim, value = strt(par3[0].limits[1]), $
 	/editable, event_pro = 'mpf_widget_perhilimpl3', xsize = '12')
 ...
 event_pro = 'mpf_widget_texmasspl3', xsize = 10) ;, /all_events

 radiobase = widget_base(fixpar, /nonexclusive)
 fixmasspl3 = widget_button(radiobase, value = 'fix', $
 event_pro = 'mpf_widget_fixmasspl3')
 
 slidermasspl3 = widget_slider(orbpar2, event_pro = $
 	'mpf_widget_slimasspl3', /suppress_value, /drag, $
 	minimum = 1, maximum = 5d5, value = initmass*100.)
 ...
 /editable, $
 event_pro = 'mpf_widget_masslowlimpl3', xsize = '12')
 ...
 uplimvalpl3 = widget_text(uplim, value = strt(par3[1].limits[1]), $
 	/editable, event_pro = 'mpf_widget_masshilimpl3', xsize = '12')
 ...
 /editable, $
 event_pro = 'mpf_widget_texeccpl3', xsize = 10)

 radiobase = widget_base(fixpar, /nonexclusive)
 fixeccpl3 = widget_button(radiobase, value = 'fix', $
 event_pro = 'mpf_widget_fixeccpl3')
 
 slidereccpl3 = widget_slider(orbpar3, event_pro = $
 	'mpf_widget_slieccpl3', /suppress_value, /drag, $
 	minimum = 1.d, maximum = 999.d, value = initecc*1000.)
 ...
 /editable, $
 event_pro = 'mpf_widget_ecclowlimpl3', xsize = '12')
 ...
 /editable, event_pro = 'mpf_widget_ecchilimpl3', xsize = '12')
 ...
 lomtextpl3 = widget_text(fixpar, value = strt(initlom), $
 	/editable, event_pro = 'mpf_widget_texlompl3', xsize = 10)

 radiobase = widget_base(fixpar, /nonexclusive)

 fixlompl3 = widget_button(radiobase, value = 'fix', $
 	event_pro = 'mpf_widget_fixlompl3')
 
 sliderlompl3 = widget_slider(orbpar6, event_pro = $
 	'mpf_widget_slilompl3', /suppress_value, /drag, $
 	minimum = 0.0, maximum = 360., value = initlom)
 ...
 lowlimvalpl3 = widget_text(lowlim, value = strt(par3[3].limits[0]), $
 	/editable, event_pro = 'mpf_widget_lomlowlimpl3', xsize = '12')
 ...
 uplimvalpl3 = widget_text(uplim, value = '360', /editable, $
 	event_pro = 'mpf_widget_lomhilimpl3', xsize = '12')
 ...
 textpar = widget_text(orbpar6, value = 'T_0 RV [d]', $
 	event_pro = 'mpf_widget_textoppl3', xsize = 10)
 
 fixpar = widget_base(orbpar6, /row)
 
 toptextpl3 = widget_text(fixpar, value = strt(inittop), $
 	/editable, event_pro = 'mpf_widget_textoppl3', xsize = 10)

 radiobase = widget_base(fixpar, /nonexclusive)

 fixtoppl3 = widget_button(radiobase, value = 'fix', $
 	event_pro = 'mpf_widget_fixtoppl3')
 
 slidertoppl3 = widget_slider(orbpar6, $
 	event_pro = 'mpf_widget_slitoppl3', /suppress_value, $
 	/drag, value = inittop, minimum = 0.d, maximum = initper + max(cf3.jd))
 ...
 lowlimvalpl3 = widget_text(lowlim, value = '1.00', /editable, $
 	event_pro = 'mpf_widget_toplowlimpl3', xsize = '12')
 ...
 uplimvalpl3 = widget_text(uplim, value = '10000', /editable, $
 	event_pro = 'mpf_widget_tophilimpl3', xsize = '12')
...
;***************TABSTUFF************************************
 ...
 resetbtn4 = widget_button(planetcontrolbase,value = 'RESET PARAMETERS',event_pro = 'mpf_widget_resetparam', xsize = 120)
 ;***************TABSTUFF************************************
 ...
 fixperpl4 = widget_button(radiobase, value = 'fix', $
 	event_pro = 'mpf_widget_fixperpl4')
 ...
 pertextpl4 = widget_text(pernums, value = strt(initper), $
 	/editable, event_pro = 'mpf_widget_texperpl4', xsize = 10)

 pertextypl4 = widget_text(pernums, value = strt(initper/365.2564d), $
 	/editable, event_pro = 'mpf_widget_texperypl4', xsize = 10)

 sliderperpl4 = widget_slider(orbpar4, event_pro = $
 	'mpf_widget_sliperpl4', /suppress_value, /drag, $
 	minimum = 1, maximum = 30000., value = initper)
 ...
 periodlowlimpl4 = widget_text(lowlim, value = $
 	...
 	'mpf_widget_perlowlimpl4', xsize = '12')
 ...
 periodhilimpl4 = widget_text(uplim, value = strt(par4[0].limits[1]), $
 	/editable, event_pro = 'mpf_widget_perhilimpl4', xsize = '12')
 ...
 event_pro = 'mpf_widget_texmasspl4', xsize = 10) ;, /all_events

 radiobase = widget_base(fixpar, /nonexclusive)
 fixmasspl4 = widget_button(radiobase, value = 'fix', $
 event_pro = 'mpf_widget_fixmasspl4')
 
 slidermasspl4 = widget_slider(orbpar2, event_pro = $
 	'mpf_widget_slimasspl4', /suppress_value, /drag, $
 	minimum = 1, maximum = 5d5, value = initmass*100.)
 ...
 /editable, $
 event_pro = 'mpf_widget_masslowlimpl4', xsize = '12')
 ...
 uplimvalpl4 = widget_text(uplim, value = strt(par4[1].limits[1]), $
 	/editable, event_pro = 'mpf_widget_masshilimpl4', xsize = '12')
 ...
 /editable, $
 event_pro = 'mpf_widget_texeccpl4', xsize = 10)

 radiobase = widget_base(fixpar, /nonexclusive)
 fixeccpl4 = widget_button(radiobase, value = 'fix', $
 event_pro = 'mpf_widget_fixeccpl4')
 
 slidereccpl4 = widget_slider(orbpar4, event_pro = $
 	'mpf_widget_slieccpl4', /suppress_value, /drag, $
 	minimum = 1.d, maximum = 999.d, value = initecc*1000.)
 ...
 /editable, $
 event_pro = 'mpf_widget_ecclowlimpl4', xsize = '12')
 ...
 /editable, event_pro = 'mpf_widget_ecchilimpl4', xsize = '12')
 ...
 lomtextpl4 = widget_text(fixpar, value = strt(initlom), $
 	/editable, event_pro = 'mpf_widget_texlompl4', xsize = 10)

 radiobase = widget_base(fixpar, /nonexclusive)

 fixlompl4 = widget_button(radiobase, value = 'fix', $
 	event_pro = 'mpf_widget_fixlompl4')
 
 sliderlompl4 = widget_slider(orbpar6, event_pro = $
 	'mpf_widget_slilompl4', /suppress_value, /drag, $
 	minimum = 0.0, maximum = 360., value = initlom)
 ...
 lowlimvalpl4 = widget_text(lowlim, value = strt(par4[3].limits[0]), $
 	/editable, event_pro = 'mpf_widget_lomlowlimpl4', xsize = '12')
 ...
 uplimvalpl4 = widget_text(uplim, value = '360', /editable, $
 	event_pro = 'mpf_widget_lomhilimpl4', xsize = '12')
 ...
 textpar = widget_text(orbpar6, value = 'T_0 RV [d]', $
 	event_pro = 'mpf_widget_textoppl4', xsize = 10)
 
 fixpar = widget_base(orbpar6, /row)
 
 toptextpl4 = widget_text(fixpar, value = strt(inittop), $
 	/editable, event_pro = 'mpf_widget_textoppl4', xsize = 10)

 radiobase = widget_base(fixpar, /nonexclusive)

 fixtoppl4 = widget_button(radiobase, value = 'fix', $
 	event_pro = 'mpf_widget_fixtoppl4')
 
 slidertoppl4 = widget_slider(orbpar6, $
 	event_pro = 'mpf_widget_slitoppl4', /suppress_value, $
 	/drag, value = inittop, minimum = 0.d, maximum = initper + max(cf3.jd))
 ...
 lowlimvalpl4 = widget_text(lowlim, value = '1.00', /editable, $
 	event_pro = 'mpf_widget_toplowlimpl4', xsize = '12')
 ...
 uplimvalpl4 = widget_text(uplim, value = '10000', /editable, $
 	event_pro = 'mpf_widget_tophilimpl4', xsize = '12')
...
;***************TABSTUFF************************************
 ...
 resetbtn5 = widget_button(planetcontrolbase,value = 'RESET PARAMETERS',event_pro = 'mpf_widget_resetparam', xsize = 120)
 ;***************TABSTUFF************************************
 ...
 fixperpl5 = widget_button(radiobase, value = 'fix', $
 	event_pro = 'mpf_widget_fixperpl5')
 ...
 pertextpl5 = widget_text(pernums, value = strt(initper), $
 	/editable, event_pro = 'mpf_widget_texperpl5', xsize = 10)

 pertextypl5 = widget_text(pernums, value = strt(initper/365.2564d), $
 	/editable, event_pro = 'mpf_widget_texperypl5', xsize = 10)

 sliderperpl5 = widget_slider(orbpar5, event_pro = $
 	'mpf_widget_sliperpl5', /suppress_value, /drag, $
 	minimum = 1, maximum = 30000., value = initper)
 ...
 periodlowlimpl5 = widget_text(lowlim, value = $
 	...
 	'mpf_widget_perlowlimpl5', xsize = '12')
 ...
 periodhilimpl5 = widget_text(uplim, value = strt(par5[0].limits[1]), $
 	/editable, event_pro = 'mpf_widget_perhilimpl5', xsize = '12')
 ...
 event_pro = 'mpf_widget_texmasspl5', xsize = 10) ;, /all_events

 radiobase = widget_base(fixpar, /nonexclusive)
 fixmasspl5 = widget_button(radiobase, value = 'fix', $
 event_pro = 'mpf_widget_fixmasspl5')
 
 slidermasspl5 = widget_slider(orbpar2, event_pro = $
 	'mpf_widget_slimasspl5', /suppress_value, /drag, $
 	minimum = 1, maximum = 5d5, value = initmass*100.)
 ...
 /editable, $
 event_pro = 'mpf_widget_masslowlimpl5', xsize = '12')
 ...
 uplimvalpl5 = widget_text(uplim, value = strt(par5[1].limits[1]), $
 	/editable, event_pro = 'mpf_widget_masshilimpl5', xsize = '12')
 ...
 /editable, $
 event_pro = 'mpf_widget_texeccpl5', xsize = 10)

 radiobase = widget_base(fixpar, /nonexclusive)
 fixeccpl5 = widget_button(radiobase, value = 'fix', $
 event_pro = 'mpf_widget_fixeccpl5')
 
 slidereccpl5 = widget_slider(orbpar5, event_pro = $
 	'mpf_widget_slieccpl5', /suppress_value, /drag, $
 	minimum = 1.d, maximum = 999.d, value = initecc*1000.)
 ...
 /editable, $
 event_pro = 'mpf_widget_ecclowlimpl5', xsize = '12')
 ...
 /editable, event_pro = 'mpf_widget_ecchilimpl5', xsize = '12')
 ...
 lomtextpl5 = widget_text(fixpar, value = strt(initlom), $
 	/editable, event_pro = 'mpf_widget_texlompl5', xsize = 10)

 radiobase = widget_base(fixpar, /nonexclusive)

 fixlompl5 = widget_button(radiobase, value = 'fix', $
 	event_pro = 'mpf_widget_fixlompl5')
 
 sliderlompl5 = widget_slider(orbpar6, event_pro = $
 	'mpf_widget_slilompl5', /suppress_value, /drag, $
 	minimum = 0.0, maximum = 360., value = initlom)
 ...
 lowlimvalpl5 = widget_text(lowlim, value = strt(par5[3].limits[0]), $
 	/editable, event_pro = 'mpf_widget_lomlowlimpl5', xsize = '12')
 ...
 uplimvalpl5 = widget_text(uplim, value = '360', /editable, $
 	event_pro = 'mpf_widget_lomhilimpl5', xsize = '12')
 ...
 textpar = widget_text(orbpar6, value = 'T_0 RV [d]', $
 	event_pro = 'mpf_widget_textoppl5', xsize = 10)
 
 fixpar = widget_base(orbpar6, /row)
 
 toptextpl5 = widget_text(fixpar, value = strt(inittop), $
 	/editable, event_pro = 'mpf_widget_textoppl5', xsize = 10)

 radiobase = widget_base(fixpar, /nonexclusive)

 fixtoppl5 = widget_button(radiobase, value = 'fix', $
 	event_pro = 'mpf_widget_fixtoppl5')
 
 slidertoppl5 = widget_slider(orbpar6, $
 	event_pro = 'mpf_widget_slitoppl5', /suppress_value, $
 	/drag, value = inittop, minimum = 0.d, maximum = initper + max(cf3.jd))
 ...
 lowlimvalpl5 = widget_text(lowlim, value = '1.00', /editable, $
 	event_pro = 'mpf_widget_toplowlimpl5', xsize = '12')
 ...
 uplimvalpl5 = widget_text(uplim, value = '10000', /editable, $
 	event_pro = 'mpf_widget_tophilimpl5', xsize = '12')
...
;***************TABSTUFF************************************
 ...
 resetbtn6 = widget_button(planetcontrolbase,value = 'RESET PARAMETERS',event_pro = 'mpf_widget_resetparam', xsize = 120)
 ;***************TABSTUFF************************************
 ...
 fixperpl6 = widget_button(radiobase, value = 'fix', $
 	event_pro = 'mpf_widget_fixperpl6')
 ...
 pertextpl6 = widget_text(pernums, value = strt(initper), $
 	/editable, event_pro = 'mpf_widget_texperpl6', xsize = 10)

 pertextypl6 = widget_text(pernums, value = strt(initper/365.2564d), $
 	/editable, event_pro = 'mpf_widget_texperypl6', xsize = 10)

 sliderperpl6 = widget_slider(orbpar6, event_pro = $
 	'mpf_widget_sliperpl6', /suppress_value, /drag, $
 	minimum = 1, maximum = 30000., value = initper)
 ...
 periodlowlimpl6 = widget_text(lowlim, value = $
 	...
 	'mpf_widget_perlowlimpl6', xsize = '12')
 ...
 periodhilimpl6 = widget_text(uplim, value = strt(par6[0].limits[1]), $
 	/editable, event_pro = 'mpf_widget_perhilimpl6', xsize = '12')
 ...
 event_pro = 'mpf_widget_texmasspl6', xsize = 10) ;, /all_events

 radiobase = widget_base(fixpar, /nonexclusive)
 fixmasspl6 = widget_button(radiobase, value = 'fix', $
 event_pro = 'mpf_widget_fixmasspl6')
 
 slidermasspl6 = widget_slider(orbpar2, event_pro = $
 	'mpf_widget_slimasspl6', /suppress_value, /drag, $
 	minimum = 1, maximum = 5d5, value = initmass*100.)
 ...
 /editable, $
 event_pro = 'mpf_widget_masslowlimpl6', xsize = '12')
 ...
 uplimvalpl6 = widget_text(uplim, value = strt(par6[1].limits[1]), $
 	/editable, event_pro = 'mpf_widget_masshilimpl6', xsize = '12')
 ...
 /editable, $
 event_pro = 'mpf_widget_texeccpl6', xsize = 10)

 radiobase = widget_base(fixpar, /nonexclusive)
 fixeccpl6 = widget_button(radiobase, value = 'fix', $
 event_pro = 'mpf_widget_fixeccpl6')
 
 slidereccpl6 = widget_slider(orbpar6, event_pro = $
 	'mpf_widget_slieccpl6', /suppress_value, /drag, $
 	minimum = 1.d, maximum = 999.d, value = initecc*1000.)
 ...
 /editable, $
 event_pro = 'mpf_widget_ecclowlimpl6', xsize = '12')
 ...
 /editable, event_pro = 'mpf_widget_ecchilimpl6', xsize = '12')
 ...
 lomtextpl6 = widget_text(fixpar, value = strt(initlom), $
 	/editable, event_pro = 'mpf_widget_texlompl6', xsize = 10)

 radiobase = widget_base(fixpar, /nonexclusive)

 fixlompl6 = widget_button(radiobase, value = 'fix', $
 	event_pro = 'mpf_widget_fixlompl6')
 
 sliderlompl6 = widget_slider(orbpar6, event_pro = $
 	'mpf_widget_slilompl6', /suppress_value, /drag, $
 	minimum = 0.0, maximum = 360., value = initlom)
 ...
 lowlimvalpl6 = widget_text(lowlim, value = strt(par6[3].limits[0]), $
 	/editable, event_pro = 'mpf_widget_lomlowlimpl6', xsize = '12')
 ...
 uplimvalpl6 = widget_text(uplim, value = '360', /editable, $
 	event_pro = 'mpf_widget_lomhilimpl6', xsize = '12')
 ...
 textpar = widget_text(orbpar6, value = 'T_0 RV [d]', $
 	event_pro = 'mpf_widget_textoppl6', xsize = 10)
 
 fixpar = widget_base(orbpar6, /row)
 
 toptextpl6 = widget_text(fixpar, value = strt(inittop), $
 	/editable, event_pro = 'mpf_widget_textoppl6', xsize = 10)

 radiobase = widget_base(fixpar, /nonexclusive)

 fixtoppl6 = widget_button(radiobase, value = 'fix', $
 	event_pro = 'mpf_widget_fixtoppl6')
 
 slidertoppl6 = widget_slider(orbpar6, $
 	event_pro = 'mpf_widget_slitoppl6', /suppress_value, $
 	/drag, value = inittop, minimum = 0.d, maximum = initper + max(cf3.jd))
 ...
 lowlimvalpl6 = widget_text(lowlim, value = '1.00', /editable, $
 	event_pro = 'mpf_widget_toplowlimpl6', xsize = '12')
 ...
 uplimvalpl6 = widget_text(uplim, value = '10000', /editable, $
 	event_pro = 'mpf_widget_tophilimpl6', xsize = '12')
...
;***************TABSTUFF************************************
 ...
 resetbtn7 = widget_button(planetcontrolbase,value = 'RESET PARAMETERS',event_pro = 'mpf_widget_resetparam', xsize = 120)
 ;***************TABSTUFF************************************
 ...
 fixperpl7 = widget_button(radiobase, value = 'fix', $
 	event_pro = 'mpf_widget_fixperpl7')
 ...
 pertextpl7 = widget_text(pernums, value = strt(initper), $
 	/editable, event_pro = 'mpf_widget_texperpl7', xsize = 10)

 pertextypl7 = widget_text(pernums, value = strt(initper/365.2564d), $
 	/editable, event_pro = 'mpf_widget_texperypl7', xsize = 10)

 sliderperpl7 = widget_slider(orbpar7, event_pro = $
 	'mpf_widget_sliperpl7', /suppress_value, /drag, $
 	minimum = 1, maximum = 30000., value = initper)
 ...
 periodlowlimpl7 = widget_text(lowlim, value = $
 	...
 	'mpf_widget_perlowlimpl7', xsize = '12')
 ...
 periodhilimpl7 = widget_text(uplim, value = strt(par7[0].limits[1]), $
 	/editable, event_pro = 'mpf_widget_perhilimpl7', xsize = '12')
 ...
 event_pro = 'mpf_widget_texmasspl7', xsize = 10) ;, /all_events

 radiobase = widget_base(fixpar, /nonexclusive)
 fixmasspl7 = widget_button(radiobase, value = 'fix', $
 event_pro = 'mpf_widget_fixmasspl7')
 
 slidermasspl7 = widget_slider(orbpar2, event_pro = $
 	'mpf_widget_slimasspl7', /suppress_value, /drag, $
 	minimum = 1, maximum = 5d5, value = initmass*100.)
 ...
 /editable, $
 event_pro = 'mpf_widget_masslowlimpl7', xsize = '12')
 ...
 uplimvalpl7 = widget_text(uplim, value = strt(par7[1].limits[1]), $
 	/editable, event_pro = 'mpf_widget_masshilimpl7', xsize = '12')
 ...
 /editable, $
 event_pro = 'mpf_widget_texeccpl7', xsize = 10)

 radiobase = widget_base(fixpar, /nonexclusive)
 fixeccpl7 = widget_button(radiobase, value = 'fix', $
 event_pro = 'mpf_widget_fixeccpl7')
 
 slidereccpl7 = widget_slider(orbpar7, event_pro = $
 	'mpf_widget_slieccpl7', /suppress_value, /drag, $
 	minimum = 1.d, maximum = 999.d, value = initecc*1000.)
 ...
 /editable, $
 event_pro = 'mpf_widget_ecclowlimpl7', xsize = '12')
 ...
 /editable, event_pro = 'mpf_widget_ecchilimpl7', xsize = '12')
 ...
 lomtextpl7 = widget_text(fixpar, value = strt(initlom), $
 	/editable, event_pro = 'mpf_widget_texlompl7', xsize = 10)

 radiobase = widget_base(fixpar, /nonexclusive)

 fixlompl7 = widget_button(radiobase, value = 'fix', $
 	event_pro = 'mpf_widget_fixlompl7')
 
 sliderlompl7 = widget_slider(orbpar6, event_pro = $
 	'mpf_widget_slilompl7', /suppress_value, /drag, $
 	minimum = 0.0, maximum = 360., value = initlom)
 ...
 lowlimvalpl7 = widget_text(lowlim, value = strt(par7[3].limits[0]), $
 	/editable, event_pro = 'mpf_widget_lomlowlimpl7', xsize = '12')
 ...
 uplimvalpl7 = widget_text(uplim, value = '360', /editable, $
 	event_pro = 'mpf_widget_lomhilimpl7', xsize = '12')
 ...
 textpar = widget_text(orbpar7, value = 'T_0 RV [d]', $
 	event_pro = 'mpf_widget_textoppl7', xsize = 10)
 
 fixpar = widget_base(orbpar7, /row)
 
 toptextpl7 = widget_text(fixpar, value = strt(inittop), $
 	/editable, event_pro = 'mpf_widget_textoppl7', xsize = 10)

 radiobase = widget_base(fixpar, /nonexclusive)

 fixtoppl7 = widget_button(radiobase, value = 'fix', $
 	event_pro = 'mpf_widget_fixtoppl7')
 
 slidertoppl7 = widget_slider(orbpar7, $
 	event_pro = 'mpf_widget_slitoppl7', /suppress_value, $
 	/drag, value = inittop, minimum = 0.d, maximum = initper + max(cf3.jd))
 ...
 lowlimvalpl7 = widget_text(lowlim, value = '1.00', /editable, $
 	event_pro = 'mpf_widget_toplowlimpl7', xsize = '12')
 ...
 uplimvalpl7 = widget_text(uplim, value = '10000', /editable, $
 	event_pro = 'mpf_widget_tophilimpl7', xsize = '12')
...
;***************TABSTUFF************************************
 ...
 resetbtn8 = widget_button(planetcontrolbase,value = 'RESET PARAMETERS',event_pro = 'mpf_widget_resetparam', xsize = 120)
 ;***************TABSTUFF************************************
 ...
 lowlimvalpl1 = widget_text(lowlim, value = '-30', /editable, $
 	event_pro = 'mpf_widget_rvolowlim', xsize = '12')
 ...
 uplimvalpl1 = widget_text(uplim, value = '30', /editable, $
 	event_pro = 'mpf_widget_rvohilim', xsize = '12')
 ...
 lowlimvalpl1 = widget_text(lowlim, value = '-100', /editable, $
 	event_pro = 'mpf_widget_srvlowlim', xsize = '12')
 ...
 uplimvalpl1 = widget_text(uplim, value = '100', /editable, $
 	event_pro = 'mpf_widget_srvhilim', xsize = '12')
 ...
 /editable, event_pro = 'mpf_widget_crvlowlim', xsize = '12')
 ...
 /editable, event_pro = 'mpf_widget_crvhilim', xsize = '12')
 ...
 lowlimvalpl1 = widget_text(lowlim, $
	 value = strt(par1[8].limits[0]), /editable, $
 	 event_pro = 'mpf_widget_dew24lowlim', xsize = '12')
 ...
 uplimvalpl1 = widget_text(uplim, $
	 value = strt(par1[8].limits[1]), /editable, $
 	 event_pro = 'mpf_widget_dew24hilim', xsize = '12')
 ...
 event_pro = 'mpf_widget_texdew39', xsize = 10)
 
 fixpardew = widget_base(orbpar9, /row)
 
 dew39text = widget_text(fixpardew, value = strt(dew39), $
 	/editable, event_pro = 'mpf_widget_texdew39', xsize = 10)

 radiobase = widget_base(fixpardew, /nonexclusive)

 fixdewpl1 = widget_button(radiobase, value = 'fix', $
 	event_pro = 'mpf_widget_fixdew39')
 
 sliderdew39 = widget_slider(orbpar9, $
 	event_pro = 'mpf_widget_slidew39', /suppress_value, $
 ...
 lowlimvalpl1 = widget_text(lowlim, $
	 value = strt(par1[9].limits[0]), /editable, $
 	 event_pro = 'mpf_widget_dew39lowlim', xsize = '12')
 ...
 uplimvalpl1 = widget_text(uplim, $
	 value = strt(par1[9].limits[1]), /editable, $
 	 event_pro = 'mpf_widget_dew39hilim', xsize = '12')
...
;**********************************************************************
 ...
 cleanup='mpf_widget_cleanup', $
 event_handler='mpf_widget_resize', $
 /no_block
