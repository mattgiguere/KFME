pro debinh,xold,yold,xnew,ynew
...
if n_params() lt 4 then begin
  print,'syntax: yold = debinh,xold,yold,xnew,ynew'
  retall
endif
...
;Extend xnew scale (xnewbig) artificially to enable rebin to xold
  ...
  ynew = hermite(x,y,xnew)          ;0th guess of ynew (finely sampled)
...
;   rebin,xnewbig,ynewbig,x,ybin         ;bin 0th guess
    ybin=resamph(double(xnewbig) $
                ,double(ynewbig) $
                ,double(x))              ;bin 0th guess
    dif  = float(ybin) - y               ;dif (binned guess - truth)
    sdif = hermite(x,dif,xnew)      ;dif splined to fine sampling
