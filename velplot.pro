...
if keyword_set(nocolor) then begin
   ...
   !x.margin=[7,5]
   !y.margin=[6,7]
   PLOT, xarr, vel, xtit=xtit, ytit=yt, titl=starname,charsize=1., $
            xr=xr, yra=yr, /xsty, /ysty, psym=8,syms=0.4*szfac, subtitle=subtt 
endif else PLOT, xarr, vel, xtit=xtit, ytit=yt, titl=starname, co=col(0),charsize=1., $
            xr=xr, yra=yr, /xsty, /ysty, psym=8,syms=0.4*szfac, subtitle=subtt 
;
;DEWAR OVERPLOT
...
device, /close
set_plot, 'x'
endif ;postscript plot because KW:filen set. 
